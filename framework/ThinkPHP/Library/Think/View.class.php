...
class View {
     ...
     * @return mixed
     */
    public function display($templateFile='',$charset='',$contentType='',$content='',$prefix='',$staticTemplateFile='') {
        G('viewStartTime');
        // 视图开始标签
        Hook::listen('view_begin',$templateFile);
        // 解析并获取模板内容
        $content = $this->fetch($templateFile,$content,$prefix,$staticTemplateFile);
        // 输出模板内容
    ...
    private function render($content,$charset='',$contentType=''){
        ...
        header('X-Powered-By:bbctop.com');
        // 输出模板文件
        echo $content;
    }
     ...
     * @return string
     */
    public function fetch($templateFile='',$content='',$prefix='',$staticTemplateFile='') {
        if(empty($content)) {
            if(empty($staticTemplateFile)){
                $templateFile   =   $this->parseTemplate($templateFile);
            }else{
                $templateFile   =   $staticTemplateFile;
            }
            // 模板文件不存在直接返回
