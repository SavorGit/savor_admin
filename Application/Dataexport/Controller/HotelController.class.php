...
where a.static_date>='$static_sdate' and a.static_date<='$static_edate' group by a.hotel_id order by a.area_id asc";
    ...
    public function getDiffExpenceHotels(){
        $city_id = I('city_id',1);
        $avg_expence = I('expence',100);
        $sql = "select a.id as hotel_id,a.name as hotel_name,a.area_id,area.region_name as area_name,a.county_id,a.business_circle_id,ext.dp_comment_num,ext.avg_expense from savor_hotel as a 
                left join savor_hotel_ext as ext on a.id=ext.hotel_id left join savor_area_info as area on a.area_id=area.id
                where a.id in (
                ...
                ) and a.state=1 and a.flag=0 and a.area_id in ($city_id)";
        $result  = M()->query($sql);
        print_r($result);
