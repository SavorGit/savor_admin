...
<form id="pagerFormaddmenuct" action="<?php echo ($host_name); ?>/menu/doaddmen" method="post" onsubmit="return validateCallback(this, dialogAjaxDone);">
	<div class="searchBar">
		<div class="clearfix">
			<!--新增节目单-->
			...
			<div id="right" style="margin-right:54px;width: 43%;background-color: #f2f2f2;float:right;margin-top: 35px;">
				...
				<input type="text" style="width: 350px;margin-top: 36px;"placeholder="名称" id="program" name="program" value="<?php echo ($menuname); ?>" <?php if($menuid > 0): ?>disabled<?php endif; ?>/>
				...
				<div class="rightlist" style="width: 94%;max-height:300px;min-height:300px;margin-left: 15px;overflow: auto;" id="righta">
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vlist): $mod = ($i % 2 );++$i;?><div id="<?php echo ($vlist["ads_id"]); ?>" dur="<?php echo ($vlist["duration"]); ?>" class="divlist2"><span class="sleft"><?php echo ($vlist["ads_name"]); ?></span><span class="sright"><?php echo ($vlist["create_time"]); ?></span></div><?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
...
*/
	...
	$("#excelsub").click(
			...
			function(){
				...
				$.ajax({
					...
					dataType: "json",
					url:"<?php echo ($host_name); ?>/menu/analyseexcel",
	...
	$('#lefta').on('click','.divlist',function(){
		if($(this).is('.active')){
			...
			if($(this).is('.active')){
				...
				$('.divlist').each(function(){
					...
					if($(this).is('.active')){
						...
						_this.before('<div id="'+ides+'" dur="'+dures+'" class="divlist2">'+$(this).html()+'</div>')
					}
				})
			}
		})
	...
	/*双击*/
	$('#lefta').on('dblclick','.divlist',function(){
		var ides = $(this).attr('id');
		var dures = $(this).attr('dur');
		$('.rightlist').append('<div id="'+ides+'" dur="'+dures+'" class="divlist2">'+$(this).html()+'</div>')
	...
	/*鼠标滑过多选*/
	$('#lefta').on('mousedown','.divlist',function(){
		...
		$.ajax({
			...
			success:function(data){
				...
				for(var i=0,l=data.length;i<l;i++) {
					...
					htm += "<div class='divlist' id="+ids+" dur="+dura+"><span class='sleft'>"+name+"</span><span class='sright'>"+cr_time+"</span></div>";
