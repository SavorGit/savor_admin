      ...
      <div class="form-group row">
        ...
        <div class="col-xs-12 col-sm-10">
          <select name="cate" class="form-control bs-select" title="请选择..." required>
            ...
            <?php if(is_array($vcainfo)): $i = 0; $__LIST__ = $vcainfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><option value="<?php echo ($row['id']); ?>" <?php if($row['id'] == $vinfo['hot_category_id']): ?>selected<?php endif; ?> > <?php echo ($row['name']); ?> </option><?php endforeach; endif; else: echo "" ;endif; ?>
		...
		<div class="form-group" style="margin-top: 30px;margin-left: 30px;">
			<!--标题-->
			<span>标题:</span>
			<input style="margin-left: 20px;" type="text" name="title" value="<?php echo ($vinfo["title"]); ?>"/>
		</div>
				...
				<div class="fileinput fileinput-new" data-fileinput="">
					<div class="fileinput-preview thumbnail" data-trigger="fileinput">
						<a data-target="#modal-file" href="javascript:void(0)">
							<img id="media_idimg" src="<?php echo ($vinfo["oss_addr"]); ?>" border="0">
		...
		<div class="pic_list" style="width: 96%;margin: 0 auto;height: 360px;margin-top: 45px;overflow: auto;border: 1px solid black;margin-bottom: 20px;">
          ...
          <?php if(is_array($detailinfo)): $i = 0; $__LIST__ = $detailinfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><div class="pic">
              <span style="float: left;margin-left: 33px;"><?php echo ($row['num']); ?></span>
              <a class="imga" data-target="#modal-file" href="<?php echo ($host_name); ?>/resource/uploadMapResource?filed=<?php echo ($row['pic_num']); ?>&rtype=2" data-browse-file>
                <img id="<?php echo ($row['pic_num']); ?>img" src="<?php echo ($row['opath']); ?>" border="0" />
              </a>
              <input type="hidden" id="<?php echo ($row['pic_num']); ?>" name="<?php echo ($row['pic_num']); ?>" value="<?php echo ($row['aid']); ?>" />
              <textarea name="<?php echo ($row['tex_num']); ?>" class="texta_val" value=""><?php echo ($row['atext']); ?></textarea>
