...
<div class="pageHeader">
  <form onsubmit="return navTabSearch(this);" id="pagerForm" action="<?php echo ($host_name); ?>/checkaccount/rplist" method="post" >
      ...
      <div class="clearfix">
        <div class="form-inline" style="margin-top:3px;">
          ...
          <div class="input-group input-group-sm pull-right">
            <a class="btn btn-success btn-sm add" href="<?php echo ($host_name); ?>/checkaccount/addcheckaccount" title="录入对账单" target="dialog" mask="true">
              <i class="fa fa-sign-in"></i><span>录入对账单</span></a>
          </div>
...
enctype="multipart/form-data" onsubmit="return iframeCallback(this, dialogAjaxDoneER)">
        <table class="table table-bordered table-striped" targetType="navTab" asc="asc" desc="desc">
          ...
          <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vlist): $mod = ($i % 2 );++$i;?><tr target="sid_user">
              ...
              <td data-title="费用类别">
                <?php echo ($vlist["cost_type"]); ?>
               </td>
              <!-- <td data-title="酒楼ID"><?php echo ($vlist["hotel_id"]); ?></td> -->
              <td data-title="费用时间段">
                <?php echo ($vlist["fee_start"]); ?>- <?php echo ($vlist["fee_end"]); ?>
               </td>
              <!-- <td data-title="区域ID"><?php echo ($vlist["area_id"]); ?></td> -->
              <td data-title="创建时间"><?php echo ($vlist["create_time"]); ?></td>
              <td data-title="发票邮寄地址"><?php echo ($vlist["receipt_addr"]); ?></td>
              <td data-title="备注"><?php echo ($vlist["stremark"]); ?></td>
            <td data-title="酒楼">

              <a data-tip="酒楼" target="navTab" title="酒楼" rel="checkaccount/showhotel" href="<?php echo ($host_name); ?>/checkaccount/showhotel?statementid=<?php echo ($vlist["statementid"]); ?>" class="btn btn-success btn-icon"><?php echo ($vlist["count"]); ?>家
              </a>

            </td>
              <td data-title="操作人"><?php echo ($vlist["uremark"]); ?> </td>
