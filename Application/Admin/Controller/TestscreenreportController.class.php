...
class TestscreenreportController extends BaseController{
	...
	public function rplist(){
		...
		if ($dtype) {
			if ( $dtype == 1) {
				$table = 'devp_mobile_statistic_year';
				$time = date("Y",time());
			} else if ($dtype == 2) {
				$table = 'devp_mobile_statistic_month';
				$time = date("Y-m",time());
			} else if ($dtype == 3) {
				$table = 'devp_mobile_statistic_date';
				$time = date("Y-m-d",time()-86400);
			} else if ($dtype == 4) {
				$table = 'devp_mobile_statistic_date_all';
				$starttime = I('post.starttime','');
				$endtime = I('post.endtime','');
			} else if ($dtype == 5) {
				$table = 'devp_mobile_statistic';
			}
		} else {
			$dtype = 3;
			$table = 'devp_mobile_statistic_date';
		...
		$screenModel =  new \Admin\Model\TestscreenRpModel($table);
