...
class CrontabController extends Controller
{
   ...
   /*  private function  umeng_api_android($info){
    ...
    public function resetnotclaimeprize(){
        $now_time = date('Y-m-d H:i:s');
        echo "resetnotclaimeprize start:$now_time \r\n";
        $m_syslottery = new \Admin\Model\Smallapp\PrizepoolprizeModel();
        $m_syslottery->handle_notclaime_prize();
        $now_time = date('Y-m-d H:i:s');
        echo "resetnotclaimeprize end:$now_time \r\n";
    }
...
//                echo $error_info.'[data]'.json_encode($res['data']);
    ...
    public function hotelstaffdata(){
        $now_time = date('Y-m-d H:i:s');
        echo "hotelstaffdata start:$now_time \r\n";
        $m_staticstaffdata = new \Admin\Model\Smallapp\StaticHotelstaffdataModel();
        $m_staticstaffdata->handle_hotel_staffdata();

        $now_time = date('Y-m-d H:i:s');
        echo "hotelstaffdata end:$now_time \r\n";
    }
	...
	public function playHotelWineAds(){
		...
		/*$box_list = array(array('box_mac'=>'00226D583ECD','http_host'=>$iphost,'http_port'=>8081),
						  array('box_mac'=>'00226D65531C','http_host'=>$iphost,'http_port'=>8081),
		                  array('box_mac'=>'00226D583D92','http_host'=>$iphost,'http_port'=>8081),
		);*/    //上线去掉
