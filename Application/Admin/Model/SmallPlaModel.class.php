...
class SmallPlaModel extends BaseModel
{
	...
	public function getWarnInfo($where, $order='id desc', $start=0,$size=5){
        ...
        FROM (SELECT max(`create_time`) tim, hotel_id FROM savor_small_platform
        group by  hotel_id) spm join
        savor_small_platform spl  on spm.hotel_id = spl.hotel_id and
        spm.tim =  spl.create_time JOIN savor_hotel sht
        ON spm.hotel_id=sht.id WHERE $where order by $order limit $start, $size";
		$list = $this->query($sql);
		$sqlb = "SELECT max(`create_time`) tim, hotel_id FROM savor_small_platform
        group by  hotel_id";
