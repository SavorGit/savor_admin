...
class BoxAwardModel extends BaseModel
{
	...
	public function getList($where, $order='id desc', $start=0,$size=5){
		$sql = "select baw.`date_time` dat,baw.`id` bawid, baw.`box_id` bid, bx.`name` bname,bx.`mac` bmac,  ro.`name` rname, ro.`type` rtp,ht.`name` hname,baw.`prize` bpr,baw.`prize_current` bpcur,baw.`flag` bflag,baw.`create_time` bcr from $this->trueTableName as `baw` left join `savor_box` bx on baw.box_id=bx.id left join `savor_room`
