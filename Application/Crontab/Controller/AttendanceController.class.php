...
class AttendanceController extends Controller{
    ...
    public function statsalerecord(){
        ...
        from savor_crm_salerecord where add_time>='$start_time' and add_time<='$end_time' and type=1 and status=2 group by ops_staff_id";
    ...
    public function statopssalerecord(){
        ...
        $sql_ops = "select a.id as ops_staff_id,a.area_id,area.region_name as area_name,a.job,suser.remark as ops_uname from savor_ops_staff as a 
            left join savor_area_info as area on a.area_id=area.id
            left join savor_sysuser as suser on a.sysuser_id=suser.id where a.job='运维工程师' and a.is_attendance=1 and suser.remark!=''
            order by a.area_id asc ";
        $res_staff = $m_attendance->query($sql_ops);
        ...
        $end_time = "$static_date 23:59:59";
        $sql = "select ops_staff_id,GROUP_CONCAT(id) as record_ids,min(signin_time) as min_signin_time,max(signout_time) as max_signout_time
            from savor_crm_salerecord where add_time>='$start_time' and add_time<='$end_time' and type=3 and status=2 group by ops_staff_id";
        $res_data = $m_attendance->query($sql);
        $salerecord_data = array();
        if(!empty($res_data)){
            foreach ($res_data as $v){
    ...
    public function statdevsalerecord(){
        $sdate = I('sdate','');
        if(empty($sdate)){
            $static_date = date('Y-m-d',strtotime('-1 day'));
        }else{
        ...
        $sql_ops = "select a.id as ops_staff_id,a.area_id,area.region_name as area_name,a.job,suser.remark as ops_uname from savor_ops_staff as a 
            left join savor_area_info as area on a.area_id=area.id
            left join savor_sysuser as suser on a.sysuser_id=suser.id where a.job='开发' and suser.remark!=''
            order by a.area_id asc ";
        $res_staff = $m_attendance->query($sql_ops);
        ...
        foreach ($all_dates as $v) {
            ...
            $sql = "select ops_staff_id,GROUP_CONCAT(id) as record_ids,min(signin_time) as min_signin_time,max(signout_time) as max_signout_time
            from savor_crm_salerecord where add_time>='$start_time' and add_time<='$end_time' and status=2 group by ops_staff_id";
            $res_data = $m_attendance->query($sql);
            $salerecord_data = array();
            if(!empty($res_data)){
                foreach ($res_data as $v){
            ...
            foreach ($res_staff as $sv){
                ...
                if(isset($salerecord_data[$ops_staff_id])){
                    ...
                    $clock_in_time = "$static_date 09:30:00";
