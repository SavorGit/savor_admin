...
class Page{
    ...
    public function __construct($totalRows, $listRows=20, $parameter = array()) {
        ...
        $request_type = IS_GET?$_GET:$_POST;
        $this->parameter  = empty($parameter) ? $request_type : $parameter;
        $this->nowPage    = empty($request_type[$this->p]) ? 1 : intval($request_type[$this->p]);
    ...
    public function admin_page($on=1){
        $totalCount = $this->totalRows;
        $numPerPage = $this->listRows;
        $currentPage = $this->nowPage;
        $page_html = "<div class='panelBar bottomBar'>";
        if($on == 1){
             $page_html .= "<div class='pages' style='padding-right:5px'><span>显示:</span></div><ul class='pagination pull-left'><li><select class='select bs-select numPerPage' data-width='4em' onChange='navTabPageBreak({numPerPage:this.value,pageNum:1})'>";
